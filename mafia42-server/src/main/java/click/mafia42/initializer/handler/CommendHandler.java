...
public class CommendHandler extends SimpleChannelInboundHandler<Payload> {
    ...
    private Payload getResponseByPayload(Payload payload, ChannelHandlerContext ctx) {
        ...
        return switch (payload.getCommend()) {
            ...
            case KICK_OUT_GAME_ROOM_USER ->
                gameRoomService.kickOutGameRoomUser(ValidationUtil.validationAndGet(payload.getBody(), KickOutGameRoomUserReq.class), ctx);
            case START_GAME ->
            ...
            case SEND_MESSAGE_TO_GAME ->
                gameRoomService.sendMessageToGame(ValidationUtil.validationAndGet(payload.getBody(), SendMessageToGameReq.class), ctx);
            case VOTE_USER ->
                gameRoomService.voteUser(ValidationUtil.validationAndGet(payload.getBody(), VoteUserReq.class), ctx);
            case VOTE_AGREE ->
                gameRoomService.voteAgree(ValidationUtil.validationAndGet(payload.getBody(), VoteAgreeReq.class), ctx);
            case VOTE_DISAGREE ->
